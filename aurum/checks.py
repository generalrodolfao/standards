...
class GitignoreHasDStore(BaseCheck):
    id = "GIT-IGNORE-002"
    message = ".gitignore não inclui .DS_Store."
...
class GitignoreHasPycache(BaseCheck):
    id = "GIT-IGNORE-003"
    message = ".gitignore não inclui __pycache__/."
    severity = Severity.WARNING
    fixable = True
...
class GitignoreHasVenv(BaseCheck):
    id = "GIT-IGNORE-004"
    message = ".gitignore não inclui .venv/."
    severity = Severity.WARNING
    fixable = True
...
class GitignoreHasEnv(BaseCheck):
    id = "GIT-IGNORE-005"
    message = ".gitignore não inclui .env."
    severity = Severity.ERROR
    fixable = True
    ...
    def run(self) -> CheckResult:
        ...
        if c is None:
            return self.fail()
        if ".env" in c and ".env.example" not in c:
            return self.ok()
...
class EnvExampleExists(BaseCheck):
    id = "GIT-ENV-001"
    message = ".env.example não encontrado."
    severity = Severity.WARNING
    fixable = True

    def run(self) -> CheckResult:
        if self._has_file(".env.example"):
            return self.ok()
...
class PcacheNotCommitted(BaseCheck):
    id = "GIT-DIRT-001"
    message = "__pycache__ rastreado pelo git."
    severity = Severity.WARNING

    def run(self) -> CheckResult:
        if not (self.project / ".git").exists():
            return CheckResult(
                id=self.id, message="Não é git repo.", severity=Severity.INFO, passed=True
            )
        try:
            r = subprocess.run(
            ...
            if r.stdout.strip():
                return self.fail(suggestion="git rm -r --cached __pycache__/")
...
PYTHON_TYPES = [
    BlueprintType.FULLSTACK_WEB,
    BlueprintType.DATA_PIPELINE,
    BlueprintType.RAG_SYSTEM,
    BlueprintType.AI_AGENT,
    BlueprintType.PYTHON_TOOL,
    BlueprintType.AGENT_SYSTEM,
]


class PyprojectExists(BaseCheck):
    id = "PY-CONFIG-001"
    message = "pyproject.toml não encontrado."
    severity = Severity.ERROR
    fixable = True
    relevant_for = PYTHON_TYPES
...
class PytestConfigured(BaseCheck):
    id = "PY-TEST-001"
    message = "pytest não configurado."
    severity = Severity.ERROR
    relevant_for = PYTHON_TYPES
    ...
    def run(self) -> CheckResult:
        ...
        if c is None:
            return self.fail()
        if "[tool.pytest.ini_options]" in c:
            return self.ok()
...
class CoverageConfigured(BaseCheck):
    id = "PY-TEST-002"
    message = "Coverage fail_under não configurado."
    severity = Severity.ERROR
    relevant_for = PYTHON_TYPES
...
class MakefileExists(BaseCheck):
    id = "PY-MAKE-001"
    message = "Makefile não encontrado."
    severity = Severity.WARNING
    fixable = True
    relevant_for = PYTHON_TYPES
...
class DockerignoreExists(BaseCheck):
    id = "DK-IGNORE-001"
    message = ".dockerignore não encontrado."
    severity = Severity.INFO
    fixable = True
    relevant_for = DOCKER_TYPES

    def run(self) -> CheckResult:
        if self._has_file(".dockerignore"):
            return self.ok()
...
class CiWorkflowExists(BaseCheck):
    id = "CI-WORKFLOW-001"
    message = ".github/workflows/ci.yml não encontrado."
    severity = Severity.WARNING
    fixable = True
    relevant_for = CI_TYPES

    def run(self) -> CheckResult:
        if self._has_file(".github", "workflows", "ci.yml"):
            return self.ok()
        return self.fail(suggestion="Crie .github/workflows/ci.yml")

    def fix(self) -> None:
        self._copy_template(
            ".github/workflows/ci.yml", self.project / ".github" / "workflows" / "ci.yml"
        )


class DependabotConfigured(BaseCheck):
    id = "CI-DEPENDABOT-001"
    message = ".github/dependabot.yml não encontrado."
    severity = Severity.INFO
    relevant_for = CI_TYPES

    def run(self) -> CheckResult:
        if self._has_file(".github", "dependabot.yml"):
            return self.ok()
...
class ClaudeMdExists(BaseCheck):
    id = "DOC-CLAUDE-001"
    message = "CLAUDE.md não encontrado (AI context)."
    severity = Severity.INFO
    fixable = True

    def run(self) -> CheckResult:
        if self._has_file("CLAUDE.md"):
            return self.ok()
...
class AdrDirExists(BaseCheck):
    id = "DOC-ADR-001"
    message = "docs/adr/ ou adr/ não encontrado."
    severity = Severity.INFO

    def run(self) -> CheckResult:
        if self._has_file("docs", "adr") or self._has_file("adr"):
            return self.ok()
...
class PreCommitConfigExists(BaseCheck):
    id = "PC-CONFIG-001"
    message = ".pre-commit-config.yaml não encontrado."
    severity = Severity.WARNING
    fixable = True
    relevant_for = PC_TYPES

    def run(self) -> CheckResult:
        if self._has_file(".pre-commit-config.yaml"):
            return self.ok()
...
class PreCommitInstalled(BaseCheck):
    id = "PC-HOOKS-001"
    message = "Pre-commit hooks não instalados."
    severity = Severity.INFO
    relevant_for = PC_TYPES

    def run(self) -> CheckResult:
        if (self.project / ".git" / "hooks" / "pre-commit").exists():
            return self.ok()
...
class LandingHtml5Doctype(BaseCheck):
    id = "LP-HTML-001"
    message = "HTML sem doctype ou <!DOCTYPE html>."
    severity = Severity.WARNING
    relevant_for = [LPT]
    ...
    def run(self) -> CheckResult:
        ...
        if not htmls:
            return self.ok("Nenhum arquivo .html encontrado")
        for h in htmls:
...
class LandingMetaViewport(BaseCheck):
    id = "LP-HTML-002"
    message = "Meta viewport não encontrado nos HTMLs."
    severity = Severity.WARNING
    relevant_for = [LPT]
...
class LandingNoVenv(BaseCheck):
    id = "LP-DIRT-001"
    message = "Landing page não deveria ter .venv/."
    severity = Severity.WARNING
    relevant_for = [LPT]
...
class LandingNoPycache(BaseCheck):
    id = "LP-DIRT-002"
    message = "Landing page não deveria ter __pycache__/."
    severity = Severity.WARNING
    relevant_for = [LPT]
...
class ToolEntryPoint(BaseCheck):
    id = "TOOL-CLI-001"
    message = "Script sem entry point (main() / cli())."
    severity = Severity.WARNING
    relevant_for = [PTT]
...
class ToolHasTests(BaseCheck):
    ...
    def run(self) -> CheckResult:
        ...
        if tests:
            return self.ok(f"{len(tests)} arquivos de teste")
        return self.fail(suggestion="Crie tests/test_tool.py")


class ToolHasReadme(BaseCheck):
    id = "TOOL-DOC-001"
    message = "README.md sem exemplos de uso."
    severity = Severity.INFO
    relevant_for = [PTT]
...
class RagVectorStore(BaseCheck):
    id = "RAG-INFRA-001"
    message = "Vector store (chroma_db/) não encontrado."
    severity = Severity.WARNING
    relevant_for = [RAGT]
...
class RagEmbeddingModel(BaseCheck):
    id = "RAG-MODEL-001"
    message = "Modelo de embedding não configurado."
    severity = Severity.WARNING
    relevant_for = [RAGT]
...
class RagPromptTemplate(BaseCheck):
    id = "RAG-PROMPT-001"
    message = "Template de prompt não encontrado."
    severity = Severity.INFO
    relevant_for = [RAGT]
    ...
    def run(self) -> CheckResult:
        ...
        for f in py_files:
            try:
                ...
                if any(
                    x in c
                    for x in ("PromptTemplate", "ChatPromptTemplate", "SystemMessagePromptTemplate")
                ):
...
class RagTestExists(BaseCheck):
    id = "RAG-TEST-001"
    message = "Testes de RAG (retrieval/qa) não encontrados."
    severity = Severity.WARNING
    relevant_for = [RAGT]
...
class AgentDefinition(BaseCheck):
    id = "AGENT-CONFIG-001"
    message = "Definição de agente não encontrada."
    severity = Severity.WARNING
    relevant_for = [AIT]
    ...
    def run(self) -> CheckResult:
        ...
        for f in py_files:
            try:
                c = f.read_text("utf-8", errors="ignore")
                if any(
                    x in c for x in ("class Agent", "AgentExecutor", "create_agent", "load_agent")
                ):
...
class AgentToolDefinition(BaseCheck):
    id = "AGENT-TOOL-001"
    message = "Tools do agente não encontradas."
    severity = Severity.INFO
    relevant_for = [AIT]
...
class AgentMemory(BaseCheck):
    id = "AGENT-MEMORY-001"
    message = "Memória do agente não configurada."
    severity = Severity.INFO
    relevant_for = [AIT]
...
class AgentTestExists(BaseCheck):
    id = "AGENT-TEST-001"
    message = "Testes do agente não encontrados."
    severity = Severity.WARNING
    relevant_for = [AIT]
...
class HasBackend(BaseCheck):
    id = "FW-BACKEND-001"
    message = "backend/ não encontrado."
    severity = Severity.WARNING
    relevant_for = [FWT]

    def run(self) -> CheckResult:
        if (self.project / "backend").is_dir():
            return self.ok()
...
class HasFrontend(BaseCheck):
    ...
    def run(self) -> CheckResult:
        if (self.project / "frontend").is_dir():
            return self.ok()
...
class HasDataDirs(BaseCheck):
    id = "DP-STRUCT-001"
    message = "Estrutura de dados não encontrada."
    severity = Severity.INFO
    relevant_for = [DPT]
    ...
    def run(self) -> CheckResult:
        found = [
            d
            for d in ("producer", "consumer", "dbt", "airflow", "api")
            if (self.project / d).is_dir()
        ]
        if found:
            return self.ok(f"Diretórios: {', '.join(found)}")
        return self.fail(suggestion="Adicione producer/, consumer/, dbt/, airflow/")


class HasDbtProject(BaseCheck):
    id = "DP-DBT-001"
    message = "dbt_project.yml não encontrado."
    severity = Severity.INFO
    relevant_for = [DPT]

    def run(self) -> CheckResult:
        if self._has_file("dbt_project.yml"):
            return self.ok()
...
class HasSquadYaml(BaseCheck):
    id = "AS-SQUAD-001"
    message = "squad.yaml não encontrado."
    severity = Severity.ERROR
    relevant_for = [AST]

    def run(self) -> CheckResult:
        if self._has_file("squad.yaml"):
            return self.ok()
...
class HasPipelineYaml(BaseCheck):
    id = "AS-PIPELINE-001"
    message = "pipeline.yaml não encontrado."
    severity = Severity.ERROR
    relevant_for = [AST]

    def run(self) -> CheckResult:
        if self._has_file("pipeline.yaml"):
            return self.ok()
...
class HasSkillsDir(BaseCheck):
    id = "AS-SKILLS-001"
    message = "skills/ não encontrado."
    severity = Severity.WARNING
    relevant_for = [AST]

    def run(self) -> CheckResult:
        if (self.project / "skills").is_dir():
            return self.ok()
